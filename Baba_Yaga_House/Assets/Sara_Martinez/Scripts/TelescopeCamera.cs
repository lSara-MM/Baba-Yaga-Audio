...
public class TelescopeCamera : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((transform.localRotation.y < 0.57f && transform.localRotation.y > -0.57f) ||
            (rawFrameVelocity.x < 0 && transform.localRotation.y >= 0.57f) || (rawFrameVelocity.x > 0 && transform.localRotation.y <= -0.57f))
        {
